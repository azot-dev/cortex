import { tracking, computeSelector, isPromise, trackSelector, isPrimitive, isFunction, isObservable, internal, optimized, isArray, getNode, findIDKey, isEmpty, isObservableValueReady, extraPrimitiveActivators, ObservablePrimitiveClass, extraPrimitiveProps, computed, observable, observe } from '@legendapp/state';
import require$$0, { useRef, forwardRef, memo, useCallback, useMemo, createElement, useEffect } from 'react';
...
function createSelectorFunctions(options) {
    ...
    return {
        ...
        run: (selector) => {
            ...
            dispose === null || dispose === void 0 ? void 0 : dispose();
            const { value, dispose: _dispose, resubscribe: _resubscribe, } = trackSelector(selector, _update, undefined, undefined, /*createResubscribe*/ true, /*inRender*/ true);
            dispose = _dispose;
...
function For({ each, eachValues, optimized: isOptimized, item, itemProps, sortValues, children, }) {
    ...
    if (eachValues) {
        ...
        if (process.env.NODE_ENV === 'development' && !internal.globalState.noDepWarn) {
            console.warn('[legend-state]: "eachValues" prop is deprecated and will be removed in version 2.0. Please use "each" prop instead.');
        }
    }
...
function Show({ if: if_, ifReady, else: else_, wrap, children }) {
    const child = useSelector(() => {
        ...
        return computeSelector((ifReady !== undefined ? isObservableValueReady(value) : value)
            ? isFunction(children)
                ? () => children(value)
                : children
            : else_
...
function Switch({ value, children, }) {
    ...
    return ((_f = (_c = (_b = (_a = children)[useSelector(value, { skipCheck: true })]) === null || _b === void 0 ? void 0 : _b.call(_a)) !== null && _c !== void 0 ? _c : (_e = (_d = children)['default']) === null || _e === void 0 ? void 0 : _e.call(_d)) !== null && _f !== void 0 ? _f : null);
}
...
function enableLegendStateReact() {
    if (process.env.NODE_ENV === 'development' && !internal.globalState.noDepWarn) {
        console.warn('[legend-state] enableLegendStateReact is deprecated and will be removed in version 2.0. Please convert rendering observables from {value} to <Memo>{value}</Memo>. See https://legendapp.com/open-source/state/migrating for more details.');
    }
    if (!isEnabled) {
        isEnabled = true;
...
export { Computed, For, Memo, Reactive, Show, Switch, configureReactive, enableLegendStateReact, hasSymbol, observer, reactive, reactiveComponents, reactiveObserver, useComputed, useEffectOnce, useIsMounted, useMount, useMountOnce, useObservable, useObservableReducer, useObserve, useObserveEffect, useSelector, useUnmount, useUnmountOnce };
